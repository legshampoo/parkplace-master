...
import { updateTagStatus, updateCurrentTag } from '../actions/actionCreators';
...
class EventHandler extends React.Component {
      ...
      try{
        ...
        if(json.status){
          ...
          if(isTagActive.length > 0){
            path = '/' + isTagActive[isTagActive.length - 1];
            updateCurrentTag(isTagActive[isTagActive.length - 1]);
            this.context.router.push(path);
          }else{
            path = '/';
            updateCurrentTag('');
        ...
        // console.log('Message is not formatted for RFID token');
      }
  ...
  apartmentRouteFn(defaultRoute, tag){
    ...
    if(this.props.tags['ap1'] && this.props.tags['ap2']){
      console.log('both are true');
      console.log(ap1Unit);
      console.log(ap2Unit);
      if(ap1Unit != 'empty' && ap2Unit != 'empty'){
    ...
    if(tag === 'ap1' && ap1Unit != 'empty'){
      return `/unit-details/${ap1Unit}`
    }else if(tag === 'ap2' && ap2Unit != 'empty'){
      return `/unit-details/${ap2Unit}`
    }else {
      return defaultRoute
    }
